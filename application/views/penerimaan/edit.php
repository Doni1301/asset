...
<body id="page-top">
				...
				<div class="row">
					<div class="col">
						<div class="card shadow">
							...
							<div class="card-body">
								<form action="<?= base_url('penerimaan/proses_edit') ?>" id="form-tambah" method="POST">
									<h5>Data Petugas</h5>
									<hr>
									<div class="form-row">
										...
										<div class="form-group col-3">
                                        	...
                                        	<input type="text" name="" value="<?= $penerimaan->kode_petugas ?? '' ?>" readonly class="form-control">
										</div>
										<div class="form-group col-3">
											<label>Nama Petugas</label>
                                        	<input type="hidden" name="nama_petugas" value="<?= $this->session->login['nama'] ?? '' ?>">
                                        	<input type="text" name="" value="<?= $penerimaan->nama_petugas ?>" readonly class="form-control">
										</div>
										<div class="form-group col-2">
										...
										<div class="col-md-10">
											...
											<div class="form-row">
												<div class="form-group col-10">
													...
													<input type="text" name="keterangan" value="<?= $penerimaan->keterangan ?>" class="form-control" autocomplete="off">
												</div>
											</div>
										</div>
									</div>
											...
											<div class="form-row">
												<div class="form-group col-12">
													<label for="nama_supplier">Nama Supplier</label>
														<input type="text" name="nama_supplier" value="<?= $penerimaan->nama_supplier ?>" class="form-control" readonly>
													</select>
												</div>
									...
									<div class="keranjang">
										...
										<table class="table table-bordered" id="keranjang">
											...
											<tbody>
											<?php foreach($all_detail_terima as $key => $dTerima): ?>
                                                <tr class="row-keranjang">
                                                    <td class="nama_barang"><?= $dTerima->nama_barang ?>
                                                        <input type="hidden" name="nama_barang_hidden[]" value="<?= $dTerima->nama_barang ?>">
                                                    </td>
                                                    <td class="kode_barang"><?= $dTerima->kode_barang ?>
                                                        <input type="hidden" name="kode_barang_hidden[]" value="<?= $dTerima->kode_barang ?>">
                                                    </td>
                                                    <td class="jumlah"><?= $dTerima->jumlah ?>
                                                        <input type="hidden" name="jumlah_hidden[]" value="<?= $dTerima->jumlah ?>">
                                                    </td>
                                                    <td class="satuan"><?= $dTerima->satuan ?>
                                                        <input type="hidden" name="satuan_hidden[]" value="<?= $dTerima->satuan ?>">
                                                    </td>
                                                    <td class="aksi">
                                                        <button type="button" class="btn btn-danger btn-sm" id="tombol-hapus" data-nama-barang="<?= $dTerima->nama_barang ?>"><i class="fa fa-trash"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
											</tbody>
	...
	<script>
		$(document).ready(function(){
            ...
            var detail_penerimaan = $( "table#keranjang tbody" ).find("button[data-nama-barang]");
            detail_penerimaan.each(function(i){
