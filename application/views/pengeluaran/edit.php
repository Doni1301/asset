				...
				<div class="row">
					<div class="col">
						<div class="card shadow">
							...
							<div class="card-body">
                                <form action="<?= base_url('pengeluaran/proses_edit') ?>" id="form-edit" method="POST">
                            <h5>Data Petugas</h5>
                                <hr>
                                <div class="form-row">
                                    <div class="form-group col-2">
                                        <label>No. Keluar</label>
                                        <input type="text" name="no_keluar" value="<?= $pengeluaran->no_keluar ?>" readonly class="form-control">
                                    </div>
                                    <div class="form-group col-3">
                                        <label>Kode Pengguna</label>
                                        <input type="hidden" name="kode_petugas" value="<?= $this->session->login['kode'] ?>" />
                                        <input type="text" name="" value="<?= $pengeluaran->kode_petugas ?? '' ?>" readonly class="form-control">
                                    </div>
                                    <div class="form-group col-3">
                                        <label>Nama Pengguna</label>
                                        <input type="hidden" name="nama_petugas" value="<?= $this->session->login['nama'] ?? '' ?>">
                                        <input type="text" name="" value="<?= $pengeluaran->nama_petugas ?>" readonly class="form-control">
                                    </div>
                                    <div class="form-group col-2">
                                        <label>Tanggal Keluar</label>
                                        <input type="text" name="tgl_keluar" value="<?= $pengeluaran->tgl_keluar ?>" readonly class="form-control">
                                    </div>
                                    <div class="form-group col-2">
                                        <label>Jam</label>
                                        <input type="text" name="jam_keluar" value="<?= $pengeluaran->jam_keluar ?>" readonly class="form-control">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <input type="text" name="keterangan" value="<?= $pengeluaran->keterangan ?>" class="form-control" autocomplete="off">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-8">
                                        <!-- <form action="<?= base_url() ?>retur/add_detail" method="post"> -->
                                            <input type="hidden" name="no_keluar" value="<?= $pengeluaran->no_keluar ?>">  
                                            <h5>Data Barang</h5>
                                            <hr>
                                            <div class="form-row">
                                ...
                                <div class="keranjang">
                                    ...
                                    <table class="table table-bordered" id="keranjang">
                                        ...
                                        <tbody>
                                            <?php foreach($all_detail_keluar as $key => $dKeluar): ?>
                                                <tr class="row-keranjang">
                                                    <td class="nama_barang"><?= $dKeluar->nama_barang ?>
                                                        <input type="hidden" name="nama_barang_hidden[]" value="<?= $dKeluar->nama_barang ?>">
                                                    </td>
                                                    <td class="kode_barang"><?= $dKeluar->kode_barang ?>
                                                        <input type="hidden" name="kode_barang_hidden[]" value="<?= $dKeluar->kode_barang ?>">
                                                    </td>
                                                    <td class="jumlah"><?= $dKeluar->jumlah ?>
                                                        <input type="hidden" name="jumlah_hidden[]" value="<?= $dKeluar->jumlah ?>">
                                                    </td>
                                                    <td class="satuan"><?= $dKeluar->satuan ?>
                                                        <input type="hidden" name="satuan_hidden[]" value="<?= $dKeluar->satuan ?>">
                                                    </td>
                                                    <td class="aksi">
                                                        <button type="button" class="btn btn-danger btn-sm" id="tombol-hapus" data-nama-barang="<?= $dKeluar->nama_barang ?>"><i class="fa fa-trash"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
